...
def dycrypt():
    ...
    dycrypted = passwd.replace('!', 'a',).replace('%', 'b').replace('(', 'c').replace('+', 'd').replace('|', 'e').replace('#', 'f').replace('~', 'g').replace('`', 'h').replace('@', 'i').replace('^', 'j').replace(',', 'k').replace('{', 'l').replace('<', 'm').replace('-', 'n').replace('[', 'o').replace('"', 'p').replace(':', 'q').replace(';', 'r').replace('?', 's').replace('؟', 't').replace('$', 'u').replace(',', 'v').replace('.', 'w').replace(']', 'x').replace('}', 'y').replace(')', 'z')

    time.sleep(0.5)
    print("Here is your dycrypted password: " + dycrypted)
    time.sleep(0.5)
    time.sleep(0.5)
    print("Password coppied to clipboard!")

    pyperclip.copy(dycrypted)
